...
import random
from .gru import *
from .hico_encoder import *

class GRU_Detect(nn.Module):
    def __init__(self,hidden_size=1024,num_class=60) -> None:
        super().__init__()
        self.register_parameter('cl_prompt', nn.Parameter(torch.zeros((1,256,)),requires_grad=False) )
        self.register_parameter('base_prompt', nn.Parameter(torch.zeros((1,1,150,)),requires_grad=False) )
...
class GRU_Detect_hico(nn.Module):
    def __init__(self,**kwards) -> None:
        super().__init__()
        self.register_parameter('cl_prompt', nn.Parameter(torch.zeros((1,256,)),requires_grad=False) )
        self.register_parameter('base_prompt', nn.Parameter(torch.zeros((1,1,150,)),requires_grad=False) )
        self.encoder_q = DownstreamEncoder(**kwards)
        #self.fc = nn.Linear(hidden_size*2,num_class)
    ...
    def forward(self, x):
        ...
        #print(T)
        #x = x.reshape(N,T,-1)# N T VCM
        logits = self.encoder_q(x)#,small_prompt=self.cl_prompt)#N,T,C
        #seq_feat = seq_feat.reshape(N*T,-1)
        #logits = self.fc(seq_feat)
        logits = logits.reshape(N,1,logits.shape[-1])
